...
(function () {
  ...
  function renderHome() {
    ...
    return [
      ...
      '<div class="emblem"><svg viewBox="0 0 80 44" width="78" height="44" fill="none" stroke="#c8a24a" stroke-width="2.6" stroke-linejoin="round"><path d="M3 22 C 18 5, 62 5, 77 22 C 62 39, 18 39, 3 22 Z"/><circle cx="40" cy="22" r="9" fill="#c8a24a" stroke="none"/><circle cx="40" cy="22" r="3.4" fill="#0f0d0b"/></svg></div>',
      '<h1>SECRET HITLER</h1>',
  ...
  function renderSetup() {
    ...
    return [
      ...
      '<div class="panel">',
      '<h3>Players</h3>',
      stepperRow('Number of players', 'playerCount', draft.playerCount, 2, 15),
      ...
      '<button class="btn primary" data-action="startGame"' + (v.ok ? '' : ' disabled') + '>Deal roles &amp; start</button>',
  ...
  function renderAdvanced(d) {
    ...
    var roleTotal = d.roles.liberals + d.roles.fascists + (d.roles.communists || 0) + 1;

    return [
      '<div class="spacer"></div>',
      '<h3>Roles</h3>',
      stepperRow('Liberals', 'roles.liberals', d.roles.liberals, 0, 14),
      stepperRow('Fascists (excl. Hitler)', 'roles.fascists', d.roles.fascists, 0, 13),
      commOn ? '<div class="kv small muted"><span>Communists</span><span>' + d.roles.communists + ' (see below)</span></div>' : '',
      '<div class="kv small muted"><span>Hitler</span><span>1 (always)</span></div>',
      '<div class="kv small"><span>Total</span><span>' + roleTotal + ' / ' + d.playerCount + '</span></div>',
      ...
      commOn ? stepperRow('Communists', 'roles.communists', d.roles.communists, 1, 6) : '',
  ...
  function renderGame() {
    ...
    return [
      topbar('Round ' + G.round, '<button class="iconbtn" data-action="viewLog">Log</button> <button class="iconbtn" data-action="openSettings" aria-label="Settings">' + GEAR_SVG + '</button> <button class="iconbtn" data-action="menu">Menu</button>'),
      renderBoard(),
      body
    ].join('');
  }
  ...
  function renderReveal() {
    ...
    if (info.knows.length) {
      knows = '<div class="note"><b>You know:</b><br>' + info.knows.map(function (k) {
        return esc(k.name) + ' — ' + esc(k.label);
      }).join('<br>') + '</div>';
    ...
    return [
      ...
      '<button class="btn primary" data-action="revealNext">Hide &amp; pass to next player</button>'
    ].join('');
  }
  ...
  function handle(action, arg, node) {
    ...
    switch (action) {
      ...
      case 'menu': if (confirm('Quit this game and return home? Progress is saved and resumable.')) { view = 'home'; render(); } break;
      ...
      case 'revealShow': ui.revealShown = true; sfx('reveal'); render(); break;
      case 'revealNext': ui.revealIdx++; ui.revealShown = false; render(); break;
      case 'beginPlay': SH.beginPlay(G); save(); ui = {}; render(); break;
  ...
  function onStep(btn) {
    ...
    if (path === 'roles.communists') {
      ...
      draft.roles.liberals = Math.max(0, draft.playerCount - draft.roles.fascists - next - 1);
    }
